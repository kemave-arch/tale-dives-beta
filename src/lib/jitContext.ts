...
// Condition Tags summary, shared by player and enemy lines below — replaces
// the old numeric HP/MP/ST vitals line entirely.
function describeConditions(conditions: ConditionTag[] | undefined): string {
  if (!conditions?.length) return 'none'
  return conditions.map((c) => c.label).join(', ')
}

// One compact line: "Skills: Shadow Step (focused) | Rend (taxing, STRAINED)".
// The STRAINED marker is the §3.2 check made visible to the narrator — its
// job is to steer toward narrating exhaustion rather than a clean cast,
...
function describeSkills(skills: Dict<SkillEntry> | undefined, player: Player): string | null {
  ...
  for (const skill of Object.values(skills)) {
    ...
    const note = [skill.effort, !affordable && `STRAINED: ${missing}`].filter(Boolean).join(', ')
...
export function buildContextSlice(state: Campaign, craftReadyLine?: string | null): string {
  ...
  // Hidden skills (§5.12) are withheld — the player hasn't discovered them,
  // so the narrator must not have them either.
  ...
  // just its own Condition Tags, restated every turn a fight is active the
