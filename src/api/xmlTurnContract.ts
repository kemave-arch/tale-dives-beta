...
<sync>
  <turn state="TURN_STATE" d="DAY_INT" h="TIME_STR" loc="LOC_ID" locdisp="LOC_DISPLAY_NAME" desc="LOC_DESC" mood="MOOD_TAG" c="±N" area="AREA_ID_or_none" areadisp="AREA_NAME" areadesc="AREA_DESC" region="REGION_ID" regiondisp="REGION_NAME" mapx="N" mapy="N" chapbeat="CHAPTER_LOG_SENTENCE" />
  ...
  <event id="EVENT_ID" stat="completed" />
  <event_trip id="EVENT_ID" />
  <project id="PROJECT_ID" stat="advanced|completed|stalled" stage="N" note="NOTE" />
  <npc id="NPC_ID" kin="parent|sibling|child|spouse|mentor|clan" aff="+|-" trust="+|-" resolve="Untrained|Novice|Adept|Expert|Master" deed="DEED_TEXT" mem="MEM_SUMMARY" wld="HELD_WEAPON" armor="WORN_ARMOR" personality="PERSONALITY_SUMMARY" appearance="APPEARANCE_SUMMARY" faction="FACTION_ID" secret="HIDDEN_TRUTH" party="companion|departed" />
...
Rules for <sync>:
- <turn> is the only always-required tag — attributes state/d/h/loc are always present; locdisp/desc/mood/c are omitted when not applicable. "d" is a bare integer only (d="1", d="47") — never prefixed with the word "Day" (d="Day 1" is invalid and discards the entire turn's sync block). locdisp follows the same "only on first visit or genuine change" economy loc_desc already has — the client already knows a visited place's display name from its own registry, so don't restate it on an ordinary same-location turn. "c" is a currency delta in base copper (e.g. c="+500", c="-1200") — omit entirely when nothing was gained or spent this turn.
- "h" is the in-fiction clock time, always 12-hour with an uppercase AM/PM suffix (h="8:20 AM", h="2:45 PM") — never bare 24-hour notation (h="20:45" is invalid). Advance "h" (and "d" across a midnight rollover) by however much time this turn's own events plausibly took, judged against what actually happened, not a reflexive one-minute nudge: a short, tense exchange of dialogue might genuinely cost only a minute or two, but real travel, a search, a rest, recovery from injury, or another character having to close real physical distance to reach the scene needs the clock to move enough to actually cover it. Never leave the clock nearly frozen turn to turn while the narration itself implies several minutes passed, an ordeal played out, or someone arrived from somewhere they weren't a moment ago — if the fiction wouldn't support how little time you just advanced, advance it further instead of leaving the contradiction standing.
- When travel between two established locations is what the clock has to account for — the player's own, or another character having to reach them — ground the required time in the real distance between them, not a flat guess: use each location's own mapx/mapy coordinates (already placed with real geographic judgment per the region-map rule below) to gauge how far apart they actually are, or, for a Tale grounded in genuine source material, that world's own real-world-equivalent distances. A short walk within the same location or its own sub-areas is quick; covering a meaningful fraction of a region's map, or the distance between two separate regions, costs proportionally real time — anywhere from tens of minutes to many hours depending on the scale and the story's own established mode of travel — never an instant hop regardless of how far apart the two points actually are. The one exception is an already-established teleportation-class means (a portal, a blink/warp/short-range-teleport skill, or an instant-travel method already narrated as this character's own) genuinely being used for that specific trip — then travel may be exactly as fast as that ability itself works, narrated accordingly, distance no longer being the limiting factor.
- <turn>'s area/areadisp/areadesc track the player's position among the CURRENT location's own named sub-areas, when the context slice shows it has any ("Areas within..."). Omit all three on an ordinary turn that doesn't move the player between areas — area, if any, silently stays whatever it already was. Set area="AREA_ID" (one already listed in "Areas within...") the turn the player's own action genuinely moves them into it. Set area="none" (a reserved literal, not a real id) the turn they step back out of a specific area into the location's general space, without leaving the location itself. To have the story discover a genuinely NEW named sub-area within the current location for the first time, invent a fresh snake_case area id paired with areadisp (its display name) and, optionally, areadesc (a short description) — never invent an area id that isn't either already listed in context or paired with a areadisp introducing it fresh. Areas never carry their own coordinates and are entirely optional — most locations and most turns have none; never invent one just to have something to report. IMPORTANT: moving into one of the current location's own areas is NEVER a new loc — e.g. if "Areas within Riders Quadrant" lists "The Mess Hall (id: the_mess_hall)" and the player heads there for a meal, keep loc="riders_quadrant" unchanged and set area="the_mess_hall" on that same <turn>; do not set loc="the_mess_hall", which would wrongly fork it into a second, disconnected top-level location.
- <turn>'s region/regiondisp/mapx/mapy place a location on its region's own local map — send them ONLY the turn a genuinely NEW loc_id (one not shown anywhere in "Known Regions"/"Known Entities" context yet) is first introduced, never restated on any later turn back through an already-registered location. region is either an already-established region's id (from "Known Regions" in context) that this new place plausibly belongs within, or a brand-new region id you're naming for the first time, paired with regiondisp (its display name) — never invent a region id that isn't either already known or paired with regiondisp introducing it fresh. mapx/mapy are 0-100 coordinates within that region's own local map, positioned with real geographic judgment relative to any other locations already placed in that same region (shown in "Known Entities" alongside their own region/xy when they have one) — closer together for a short journey, farther apart for a long one, never arbitrary or centered by default. Ground the scale itself in something real: when this Tale draws on genuine source material (see the Lore Accuracy Contract, when present), use that world's own actual real-world geographic proportions; otherwise treat one region as roughly the size of a real-world country or province — ordinary Earth-like distances — so a single day's travel should only nudge a coordinate a small fraction of the 0-100 range, never leap most of the way across it.
- <turn>'s "chapbeat" is ONE short, past-tense, plot-relevant sentence for this turn's own entry in a running chapter log (e.g. "Kael discovered the merchant's hidden ledger and confronted him about the smuggled goods.") — set it ONLY on a turn that genuinely advances the story (a real decision, discovery, confrontation, or turning point), never on an ordinary or purely atmospheric turn; most turns should omit it entirely. Write it as a standalone fact a reader skimming just this log would understand — never "he did it again" or anything assuming prior sentences. Feeds the player-facing chapter timeline directly; only once a chapter closes does a compact digest of these sentences ever come back to you, as "Story So Far" for a LATER chapter — never restated to you within the same still-open chapter.
...
- <item>, <corpse>, <flag>, <npc>, <fac>, <skill>, <enrich>, <project>, <event>, <event_trip> may each repeat 0 or more times — one tag per item/enemy/flag/NPC/faction/skill/entity/project/event/tripped event affected this turn.
...
- <quest>'s "stat" (advanced/completed/failed — always the full word, never abbreviated, and never "active": a brand-new quest's very first stat is "advanced" too, exactly like progressing one already underway — "active" belongs to <beat> instead, a different tag with its own separate vocabulary) and "type" (main/side/ambition/secret_ambition) follow the same "only on first introduction" economy as desc for type — omit type on an ordinary advancement turn for a quest already introduced. A secret_ambition quest only ever originates or advances on a turn whose <turn> state is INSIGHT or EXPLORE.
- <npc>'s "kin" is OPTIONAL and sets or revises structural kinship (parent|sibling|child|spouse|mentor|clan) — parent, sibling, and child permanently hard-gate against romantic/intimacy escalation (Rule 5a) while allowing deep familial affection.
...
- <npc>'s "appearance" is the same discipline as "personality" — a short, concrete physical anchor (build, coloring, a scar, a distinguishing feature or habit of dress), set once when an NPC becomes genuinely important to the scene and restated every turn thereafter as ground truth. Anchor it in something SPECIFIC to this one person, never a generic descriptor that could belong to any character in this cast (a recurring failure mode: giving every notable NPC the same eye color or build) — when two present NPCs would otherwise share a trait, differentiate them on purpose. Never restate it as a full physical re-description in the prose every turn; it exists so you have a stable, distinguishing detail to reach for when it actually matters (a glance, a reaction, someone recognizing them), not a mandatory recap.
...
- The instant the scene moves on from an NPC — the protagonist walks away, the location changes, the conversation ends — and that NPC did NOT get party="companion", they are no longer present. Stop including an <npc> tag for them: no habitual restating of aff/trust/personality/secret "for continuity," even if they were central to the last several turns. An <npc> tag this turn is a claim that this person is here, right now; an NPC touched out of habit after the scene has already left them behind is exactly what makes them wrongly linger — chiming in, reacting, or getting a "final word" on turns they were never actually still in the room for. Let them go quiet until the story itself genuinely brings the protagonist back into their presence.
...
- <event> marks an already-ACTIVE Narrative Event (shown in context as "Active Narrative Events" only once it has actually activated — you never see a dormant one, so never reference or invent one that isn't currently listed there) as "completed" once its own guidance has genuinely played out on the page. "stat" only ever takes the value "completed" — a Narrative Event's activation is entirely client-tracked, never something you set yourself. At most one <event> per completed event per turn; omit entirely on a turn that doesn't resolve one.
- <event_trip> trips a DORMANT Narrative Event whose trigger condition was listed in the context slice under "Dormant Story Watches: [EVENT_ID]: CONDITION". Emit <event_trip id="EVENT_ID" /> ONLY when your narrative prose in <nar> genuinely and unmistakably satisfied that exact narrative condition on this turn. Emitting this trips the event from dormant to active; on subsequent turns you will receive its full narrative guidance under "Active Narrative Events". Never emit <event_trip> for an event not explicitly listed in "Dormant Story Watches".
- <end> appears ONLY on a turn whose Player Action line reads exactly "!conclude", OR the same turn a <beat> completes the LAST beat in the pre-authored Story Arc — never emit it otherwise, no matter how final or climactic the scene reads. "outcome" is required whenever <end> appears (win/lose/neutral, see rule 2d) — this is the Tale's real final scene, not a hook for the next one. When context shows "Ending Guidance," honor the instructions for whichever outcome you choose while narrating this final scene.
