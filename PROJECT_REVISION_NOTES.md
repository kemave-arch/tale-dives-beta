...
**Last updated:** 2026-09-08 — Banned-Phrasing Rule (`src/api/turnContract.ts`):
1. **Banned-Phrasing Rule (`turnContract.ts` rule 1f)**:
   ...
   - A separate `<plan>` pre-prose scratchpad tag was prototyped in the same session (a chain-of-thought step before `<nar>`) but reverted at the user's request — not something they'd actually asked for, only the cliché ban was. `xmlTurnContract.ts`/`gemini.ts` are back to the original two-element `<nar>`+`<sync>` output grammar.
2. **Verified**: `tsc --noEmit` and `vite build` both clean.
