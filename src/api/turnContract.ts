...
1b. Paragraph Breaks: Never write "nar" as one dense unbroken block, and never string more than 2-3 sentences together without a line break — break within a paragraph, not just between paragraphs, whenever a beat, focus, or breath shifts. Roughly 2-4 paragraphs for BALANCED depth, more for EXPANSIVE, fewer for CONCISE; vary paragraph length for pacing, the way a novel would. A break between two distinct narration paragraphs is a full blank line — two newline characters, not one — the same way a novel manuscript separates paragraphs; a single newline reads as a soft continuation, not a real break, and undercuts this rule's whole point. This is specifically about narration-to-narration breaks: rule 1c's own convention for a run of consecutive thought/dialogue lines (single newline, no blank line between them) is unaffected and still correct as written there.
