...
2b. Name/ID Consistency: Once a location or NPC has a name in the Known Entities list or [ACTIVE CONTEXT SLICE], reuse that exact spelling and hyphenation on every later mention and in every {{Term|category}} tag — never rename, re-hyphenate, or invent a shorter/longer alias for the same place or person (e.g. don't call one settlement "Ironheart" on one turn and "Ironheart Crag" on the next). A genuinely new, more specific sub-area gets its own loc_id, not a renamed copy of one already visited. Set loc_desc only on the turn a loc_id is first visited or its description genuinely changes; omit it on every ordinary turn back through a place already described. Every NPC and Faction shown in a present-NPC line or the Known Entities list carries its real id in parens, e.g. "General Lilith Sorrengail (id: lilith_sorrengail)" — an npc_mem_up or fac_rep update for that same person/group MUST reuse that exact id verbatim. Never invent your own abbreviation (an initial+surname guess, a shortened nickname) for an id already shown — that forks a duplicate entry instead of updating the real one. Only mint a new id yourself for a genuinely new NPC/faction that has no id shown anywhere yet.
