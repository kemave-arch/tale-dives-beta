...
- **2026-09-06** — Mobile Settings Sheet Overhaul & Reliable Google Drive Mobile Auth Flow (`src/screens/Settings.tsx`, `src/lib/googleDrive.ts`, `src/lib/glassChrome.tsx`, `src/screens/TaleBrief.tsx`, `src/App.tsx`):
  ...
  - **Reliable Google Drive Auth on Mobile (`src/lib/googleDrive.ts`, `src/App.tsx`)**: Addressed popup blocking and flicker failures on mobile browsers by implementing a redirect flow fallback via Firebase Auth (`signInWithRedirect` on mobile detection or popup block). Added `completeGoogleRedirectSignIn()` called once during app initialization in `App.tsx` (`useEffect`) to retrieve auth results on return redirect. Handled intentional popup closures (`auth/popup-closed-by-user`) gracefully without throwing unhandled exceptions.
