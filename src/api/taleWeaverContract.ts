...
<phase>
  ...
  <protagonist name="NAME" aliases="OPTIONAL_TITLES_OR_NICKNAMES" gender="male|female" background="BACKGROUND" personality="PERSONALITY" motivation="MOTIVATION" physical_trait="PHYSICAL_TRAIT" secret="SECRET" opening="OPENING_SCENE" class="CLASS_OR_ARCHETYPE" str="Untrained|Novice|Adept|Expert|Master" int="Untrained|Novice|Adept|Expert|Master" agi="Untrained|Novice|Adept|Expert|Master" />
...
- World Foundation phase: exactly one <world> tag with EVERY attribute filled with rich, specific content. Always include era_tech specifying the historical era and technology level (e.g. "Late Medieval / Iron Age", "Victorian Gaslamp / Steampunk", "Far-Future Spacefaring"), power_system, genre_tone, conflict, key_factions, and background. When a Lore Accuracy Contract is active (see the system instructions above), power_system and conflict are NOT free invention slots — if the source material establishes a real power system (magic system, technology, ability set) or a real central conflict, describe that actual one accurately rather than inventing an original-sounding substitute; invent only the details a real source genuinely never specified.
- Protagonist phase: exactly one <protagonist> tag with every attribute populated — "aliases" is the one exception, left empty unless the character's own lore genuinely gives them a title, epithet, or nickname the world calls them by (e.g. "the Shadowblade", "Ashveil's Ghost") — never invent one just to fill the slot. Also including "gender" (male or female, your own best fit for the described protagonist, used purely to pick matching cover art, never narrated or treated as a trait) — and a fitting "class" (a short evocative class/archetype name — e.g. "Warrior", "Mage", "Dragon Rider", "Necromancer", "Scribe" — or an original one fitting the world's own power system; never leave it blank). Also set "str"/"int"/"agi" to whichever tier word actually fits this specific character's background, physical_trait, and class — a fragile, bookish, or untrained character should start with a genuinely low tier (Untrained/Novice) on the attributes their concept doesn't support, not a flattering average; a seasoned fighter starts high STR; a frail scholar starts low STR/AGI and high INT. These are a lore-accurate starting point, not a balanced stat spread. Also include 0-3 <skill> tags for starting abilities that genuinely make sense given the protagonist's background and class — omit entirely for a mundane, non-abilities-based character concept rather than inventing one that doesn't fit.
...
it being fabricated. This extends to the world's own premise, not just its cast and places: if the
source establishes a real power system (a magic system, technology, superhuman ability, or any
governing rule of how the fictional world works) or a real central conflict, describe that actual one
— e.g. Spider-Man's powers come from a radioactive/genetically-altered spider bite, not an invented
substitute technology — rather than dressing up an original-sounding replacement. Only invent a
power_system or conflict outright when the source genuinely never establishes one of its own.
