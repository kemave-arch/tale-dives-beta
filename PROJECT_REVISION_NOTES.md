...
**Last updated:** 2026-09-08 — Pre-Prose Planning Scratchpad & Banned-Phrasing Rule (`src/api/turnContract.ts`, `src/api/xmlTurnContract.ts`, `src/api/providers/gemini.ts`):
1. **`<plan>` Pre-Prose Scratchpad (chain-of-thought before creative output)**:
   - Added a new `<plan>` tag to the XML output grammar, placed BEFORE `<nar>` (the model now emits `<plan>` → `<nar>` → `<sync>`, three top-level elements instead of two).
   - `<plan>` is a short, private scratchpad the model fills in first — tone/craft direction for the beat, 2-3 concrete sensory details it intends to actually use, and the one thing that moves (a reveal, an NPC reaction, a plot beat) — so `<nar>` delivers what was just planned instead of the model composing continuity and prose simultaneously. This is a known technique for getting better creative output out of a fast/lite model (Gemini 3.5 Flash-Lite is the app's configured default): forcing an explicit "think before you write" step measurably helps a weaker model more than a top-tier one.
   - `<plan>` is pure scratch — never parsed into `TurnResponse` (no schema field), never shown to the player, and stripped out of `history` by `stripSyncForHistory` in `gemini.ts` (same treatment as `<sync>`) so it isn't resent as growing context on every later turn — it only costs output tokens on the turn that produced it.
   - **Deliberate tradeoff, flagged in `xmlTurnContract.ts`'s own comments**: `<nar>` was originally placed FIRST specifically so a `MAX_TOKENS` truncation mid-response still preserves the prose (the Stage 3 Fallback Reader in `gemini.ts` recovers whatever `<nar>` text made it out). Putting `<plan>` before `<nar>` means a truncation landing mid-`<plan>` now loses the turn's prose entirely instead of just its trailing `<sync>` deltas. Accepted because `<plan>` is instructed to stay short against a per-turn budget already floored at `MIN_TURN_OUTPUT_CEILING` (6144 tokens) regardless of Prose Depth — this failure mode should be rare — and because the player explicitly prioritized maximum narrative/creative quality over API cost or margin-of-safety here.
   - Verified all downstream regex/parsing paths tolerate the new leading tag without changes: `<nar>` extraction (`xmlTurnParser.ts`, `gemini.ts`'s `extractXmlNarrative`), `<sync>` block extraction (`xmlHelpers.ts`'s `parseXmlBlock`, `Chronicle.tsx`'s `extractSyncBlock`), and the debug-payload tools (`DebugPayloadButton`, `SessionPayloadPanel`, `buildSessionPayloadText`) — all match on `<nar>`/`<sync>` directly regardless of what precedes them, and the debug tools dump the full raw response anyway, so `<plan>` becomes visible there too (useful for judging whether the planning step is actually helping).
2. **Banned-Phrasing Rule (`turnContract.ts` rule 1f)**:
   - Added an explicit list of overused AI-fantasy stock phrases the model must never use (or close variants of): "delve/delved", "shrouded in mystery", "testament to", "a force to be reckoned with", "in the tapestry of", "beckoned", "loomed ominously", "sent a shiver down [their] spine", "a stark reminder", "little did [they] know", "the air was thick with", "with bated breath", "against all odds", "a dangerous game", "as the old adage goes" — with an instruction to replace any of these with something concrete and scene-specific instead.
   - Lives in the static `SYSTEM_INSTRUCTIONS` string, so it's part of the cached prompt prefix — effectively free under prompt caching, no added per-turn cost.
3. **Verified**: `tsc --noEmit` and `vite build` both clean. Regex-level smoke test confirmed a leading `<plan>` block doesn't break `<nar>` extraction, `<sync>` block extraction, the history-stripping regex, or either debug-payload tool's parsing.
